import { useMemo } from 'react'
import {
  formatChlorophyllTick,
  getChlorophyllGradientCss,
  getChlorophyllLegendTicks,
  type ChlorophyllRange,
} from '../../utils/chlorophyllColor'

interface ChlorophyllColorbarProps {
  range: ChlorophyllRange
  unit?: string
  visible?: boolean
}

export function ChlorophyllColorbar({
  range,
  unit = 'mg/m³',
  visible = true,
}: ChlorophyllColorbarProps) {
  const ticks = useMemo(
    () => getChlorophyllLegendTicks(range.min, range.max),
    [range.min, range.max],
  )

  if (!visible) return null

  return (
    <div className="temp-colorbar">
      <span className="temp-colorbar__title">
        CHLOROPHYLL <span className="temp-colorbar__unit">{unit}</span>
      </span>
      <div className="temp-colorbar__body">
        <div className="temp-colorbar__labels">
          <span className="temp-colorbar__extreme">high</span>
          {ticks.map((tick) => (
            <span key={tick} className="temp-colorbar__tick">
              {formatChlorophyllTick(tick)}
            </span>
          ))}
          <span className="temp-colorbar__extreme">low</span>
        </div>
        <div
          className="temp-colorbar__gradient"
          style={{ background: getChlorophyllGradientCss('vertical') }}
          aria-hidden
        />
      </div>
    </div>
  )
}
